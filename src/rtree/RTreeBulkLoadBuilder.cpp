...
namespace rtree {
    ...
    RTreeBulkLoadBuilder::RTreeBulkLoadBuilder() :
        m_maxNodeEntries(DEFAULT_MAX_NODE_ENTRIES),
        m_minNodeEntries(DEFAULT_MIN_NODE_ENTRIES)
    {
    }

    void RTreeBulkLoadBuilder::bulkLoad(std::vector<Rectangle>& rectangles) {
        // Clear existing tree
        m_nodeMap.clear();
        m_size = 0;
        m_treeHeight = 0;
        ...
        auto leafNodes = createLeafs(rectangles, optimalFill);
    ...
    std::vector<std::shared_ptr<Node>> RTreeBulkLoadBuilder::createLeafs(std::vector<Rectangle>& rectangles, int nodeCapacity) {
        std::vector<std::shared_ptr<Node>> leafNodes;
        leafNodes.reserve(rectangles.size() / nodeCapacity + 1);
    ...
    std::vector<std::shared_ptr<Node>> RTreeBulkLoadBuilder::createNextLevel(std::vector<std::shared_ptr<Node>>& nodes, int nodeCapacity) {
    ...
    std::shared_ptr<Node> RTreeBulkLoadBuilder::createNode(const std::vector<std::shared_ptr<Node>>& children, int level) {
    ...
    std::shared_ptr<Node> RTreeBulkLoadBuilder::createLeafNode(const std::vector<Rectangle>& rectangles, int start, int end) {
    ...
    int RTreeBulkLoadBuilder::getNextNodeId() {
        return m_nextNodeId++;
    }

    std::shared_ptr<Node> RTreeBulkLoadBuilder::getNode(int id) {
        return m_nodeMap.at(id);
    }

    int RTreeBulkLoadBuilder::treeSize() const {
        return m_size;
    }

    int RTreeBulkLoadBuilder::numNodes() const {
        return m_nodeMap.size();
    }

    int RTreeBulkLoadBuilder::getRootNodeId() const {
        return m_rootNodeId;
    }
